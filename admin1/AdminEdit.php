...
include '../config.php';
  if (isset($_GET['del_id'])) {
    ...
    $deleteQuery = "DELETE  FROM `admin_tbl`WHERE `AdminID`='$id' ";
...
if (isset($_POST['patientBtnUpdate'])) {
    ...
    if ($conn) {
        if (empty($_FILES['img']['name'])) {
            $PatientUpdateQuery = "UPDATE `Admin_tbl` SET `AdminName`='$name',`AdminEmail`='$email',`AdminPassword`='$pass',`AdminContact`='$contact' WHERE `AdminID`='$id'";
...
if (isset($_GET['p_id'])) {
    $id = $_GET['p_id'];
    if ($conn) {
        $findPatient = "SELECT * FROM `admin_tbl`WHERE `AdminID`='$id' ";
